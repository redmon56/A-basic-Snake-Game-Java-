...
public class GameSpace extends JFrame implements Runnable {
	...
	class Panel extends JPanel{
		...
		public Panel(){
			...
			temp = new JPanel(new GridLayout(1,3));
			{
				...
				{
					label = new JLabel("By Redmon56",SwingConstants.CENTER);
						label.setFont(new Font("Courier",0,16));
					end.add(label);
					label = new JLabel("Ver. 2.0",SwingConstants.CENTER);
						label.setFont(new Font("Courier",0,16));
					end.add(label);
				}
				temp.add(end);
	...
	//Method: update() -- Type void
	//	Updates the game and each unit of the
	//	snake
	void update(){
	...
	//Method: testSnakeHead() -- Type boolean
	//	Checks to make sure the snake is still in the
	//	games boundaries and that it has not gone
	//	inside itself
